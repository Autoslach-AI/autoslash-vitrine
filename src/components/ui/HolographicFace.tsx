  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      particles.current.forEach(p => {
        ...
        const opacity = 0.2 + Math.sin(tRef.current * (0.5 / p.speed) + p.offset) * 0.25 + 0.15;

        ctx.beginPath();
        ctx.arc(px, py, p.r, 0, Math.PI * 2);
        ctx.fillStyle = color + Math.floor(Math.min(opacity, 0.6) * 255).toString(16).padStart(2, "0");
        ctx.fill();
      });
