...
import { Heart, User, Eye, ArrowRight, Star } from "lucide-react";
  ...
  return (
    ...
    >
      <Link 
        ...
        onClick={saveContext}
        className="relative aspect-[4/3] rounded-xl overflow-hidden bg-neutral-100 border border-black/[0.03] cursor-pointer block"
      >
        <img 
          src={template.image} 
          alt=""
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-700 flex items-center justify-center p-8 bg-black/5 backdrop-blur-[1px]">
          <div className="text-[#2a6df5] text-[11px] font-black uppercase tracking-widest bg-white/10 backdrop-blur-md px-4 py-2 rounded-full border border-white/10 shadow-xl">
            {template.price ? `${template.price.toLocaleString('fr-FR', { useGrouping: true }).replace(/\s/g, ' ')} FCFA` : 'Sur devis'}
          </div>
        </div>
      </Link>
      ...
      <div className="flex items-start justify-between px-1">
        ...
        <div className="flex items-center gap-2.5 text-white/20">
          <div className="flex items-center gap-1 group/like cursor-pointer">
            <Heart className="w-3.5 h-3.5 group-hover/like:text-red-500 transition-colors" />
